...
import { asNumber, canonicalUrl, fetchJson, parseSizeGrams, userAgent } from "./product-utils.js";
import { loadEnv } from "./env.js";
import { syncProducts } from "./supabase-history.js";
import { discoveryConfigs } from "./retailer-config.js";
...
export async function discoverProducts(options = {}) {
  ...
  for (const config of discoveryConfigs) {
    try {
      const products = await discoverFromConfig(config, current.products);
  ...
  const retainedProducts = current.products.filter(
