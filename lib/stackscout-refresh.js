...
async function refreshSafely(reason) {
  ...
  try {
    ...
    const seedProducts = (await readProductDataset()).products;
    const discovery = await discoverProducts({ products: seedProducts });
    const result = await refreshProducts({ products: discovery.products });
