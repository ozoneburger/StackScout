...
import { syncPriceHistory } from "../scripts/supabase-history.js";
...
export async function refreshProducts(options = {}) {
  if (!Array.isArray(options.products)) {
    throw new Error("refreshProducts requires a products array");
  }
  const refreshConcurrency = options.concurrency ?? Number(process.env.FETCH_CONCURRENCY ?? 2);
  const products = await mapWithConcurrency(options.products, refreshConcurrency, fetchOne);
